'use client'
// Full-screen confirmation shown after checkout — handles both online and offline saves
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center px-6 text-center">
      ...
      <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 ${
        offline ? 'bg-amber-500/20' : 'bg-green-500/20'
      }`}>
        {offline
      ...
      {/* Offline notice */}
      {offline && (
        <p className="text-sm text-amber-400 mb-3 max-w-xs">
          No internet right now. Sale is queued and will sync automatically when you&apos;re back online.
        </p>
      )}

      {/* Total */}
      <p className={`text-5xl font-bold mb-3 ${offline ? 'text-amber-400' : 'text-green-400'}`}>
