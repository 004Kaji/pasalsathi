...
export default function LandingPage() {
  ...
  const t = T[lang]
  // Check localStorage for a Supabase session — no network call, instant
  const [loggedIn, setLoggedIn] = useState(false)
  useEffect(() => {
    setLoggedIn(Object.keys(localStorage).some(k => k.includes('-auth-token')))
  }, [])
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#F5F0E8]/90 backdrop-blur-xl border-b border-[#D5CFC6]">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {loggedIn ? (
              <Link href="/sell" className="text-sm bg-[#C84B2F] hover:opacity-90 text-white px-4 py-2 rounded-xl font-semibold transition-all active:scale-95">
                Open App →
              </Link>
            ) : (
              <>
