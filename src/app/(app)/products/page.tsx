...
export default function GodamPage() {
  ...
  const lowStockProducts = physicalProducts.filter(
    p => Number(p.stock) < Number(0)
  )

  const filtered = activeList.filter(p => {
    const matchSearch = p.name.toLowerCase().includes(search.toLowerCase())
    const matchFilter = filter === 'all' || Number(p.stock) < Number(0)
    return matchSearch && (tab === 'service' ? matchSearch : matchFilter)
  })

  const totalValue = physicalProducts.reduce(
    (sum, p) => sum + Number(p.stock) * Number(0), 0
  )

  const atLimit = false // no plan limits in simplified schema
...
function ProductCard({ product }: { product: Product }) {
  const isLow = false // low_stock_threshold removed from simplified schema
