    ...
    <div className="pb-6">
      ...
      <div className="sticky top-0 bg-white border-b border-gray-100 z-10 px-4 pt-5 pb-3">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">स्टाफ</h1>
          <div className="flex gap-2">
            {staffList.length > 0 && (
              <Link
                href="/staff/attendance"
                className="flex items-center gap-1.5 bg-purple-600 text-white px-4 py-2.5 rounded-xl font-semibold text-sm active:scale-95 transition-transform"
              >
            ...
            {!atLimit && (
              <Link
                href="/staff/new"
                className="flex items-center gap-1.5 bg-orange-600 text-white px-4 py-2.5 rounded-xl font-semibold text-sm active:scale-95 transition-transform"
              >
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="bg-purple-600 rounded-2xl p-5">
          <p className="text-purple-100 text-sm">{todayDisplay}</p>
          <div className="flex items-end justify-between mt-2">
            <div>
              <p className="text-purple-100 text-sm font-medium">आज उपस्थित</p>
              <p className="text-white text-4xl font-bold mt-0.5">
                {presentCount}
                <span className="text-2xl text-purple-300">/{staffList.length}</span>
              </p>
            </div>
            <div className="text-right">
              <UserCheck size={40} className="text-purple-300" />
        ...
        {limit !== Infinity && (
          <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
            <span className="text-sm text-gray-600">स्टाफ: <strong>{staffList.length}</strong> / {limit}</span>
        ...
        {staffList.length > 0 && markedCount < staffList.length && (
          <Link href="/staff/attendance">
            <div className="bg-yellow-50 border-2 border-yellow-300 rounded-2xl p-4 flex items-center justify-between active:scale-[0.98] transition-transform">
              <div>
                <p className="font-bold text-yellow-800 text-base">आजको हाजिरी भर्नुहोस्</p>
                <p className="text-yellow-600 text-sm mt-0.5">
                  {staffList.length - markedCount} जनाको हाजिरी अझै भरिएको छैन
                </p>
              </div>
              <ChevronRight size={22} className="text-yellow-600" />
            </div>
          </Link>
        )}
            ...
            {staffList.map((s) => {
              ...
              return (
                <Link key={s.id} href={`/staff/${s.id}`}>
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center justify-between active:scale-[0.98] transition-transform">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="bg-purple-100 rounded-full w-12 h-12 flex items-center justify-center shrink-0">
